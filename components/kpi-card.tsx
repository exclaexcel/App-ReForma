...
export function KpiCard({ label, value, icon: Icon, variant = "default" }: KpiCardProps) {
  return (
    <div className={cn(
      "rounded-xl border p-4 flex flex-col gap-2 shadow-sm shadow-black/20 transition-all duration-200 group hover:border-zinc-600/80 dark:hover:bg-zinc-700/50 hover:bg-stone-200/50",
      variantStyles[variant]
    )}>
      <div className="flex items-center justify-between">
        <span className="text-xs dark:text-zinc-400 text-stone-600 font-semibold uppercase tracking-wider">{label}</span>
        <Icon className={cn("h-4 w-4", iconStyles[variant])} />
      </div>
      <span className="text-xl font-bold dark:text-zinc-100 text-stone-900 tabular-nums">
