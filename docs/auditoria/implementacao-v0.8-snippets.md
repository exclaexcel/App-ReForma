  ...
  daysElapsed !== null ? (
    ...
    <div className="rounded-xl bg-stone-800/30 border border-stone-700 px-4 py-3">
      <p className="text-sm text-stone-400">
        ...
        <Link href="/projeto/editar" className="text-orange-600">
          Configurar
        </Link>
      </p>
    </div>
...
export function ExpenseListItem({ expense, href }: Props) {
  ...
  return (
    <Link
      href={href}
      className="flex items-center justify-between py-3 px-3 border-b border-stone-200 dark:border-zinc-800"
    >
      {/* conteúdo existente */}
...
const saldoBgColor =
  saldo >= 0 ? "bg-emerald-900/20 border-emerald-800" : "bg-red-900/20 border-red-800";
```
